...
from glue_eval.useful_functions import load_data, load_data_split, MODEL_NAME_TO_MAXIMUM_CONTEXT_LENGTH_MAP, get_llama_without_answer, get_qwen_without_answer
import time
import torch
import numpy as np
...
class DIALOGUE_Eval():
    ...
    def _create_prompt(self, example, gen_len):
        ...
        input_prompt = get_llama_without_answer(input_prompt) if "llama" in self.model.config._name_or_path.lower() else get_qwen_without_answer(input_prompt)
    ...
    def _get_answer(self, generated_text):
        ...
        if 'a\n' in generated_text.lower() or 'a)' in generated_text.lower()  or '(a' in generated_text.lower():
            return 0
        if 'b\n' in generated_text.lower() or 'b)' in generated_text.lower() or '(b' in generated_text.lower():
            return 1
        if 'c\n' in generated_text.lower() or 'c)' in generated_text.lower() or '(c' in generated_text.lower():
            return 2
        if 'd\n' in generated_text.lower() or 'd)' in generated_text.lower() or '(d' in generated_text.lower():
            return 3
        return -1
    ...
    def evaluate(self, gen_len = 3, print_logs = False):
        ...
        for s, example in enumerate(self.eval_dataset):
            ...
            print(input_prompt)
            input_prompt_ids = self.tokenizer.encode(input_prompt, return_tensors='pt').to(self.model.device)
            input_prompt_text = self.tokenizer.decode(input_prompt_ids[0], skip_special_tokens=False)
            ...
            for i in range(len(suffixes.keys())):
                prompt_tok = self.tokenizer([f"{input_prompt} {suffixes[i][0]}"], return_tensors="pt").to(self.model.device)
