...
def apply_memit_ARE_to_model(
    ...
    **kwargs
    ...
    batch_question_ans = [
        i['prompt'] + i['target_new'] for i in batch_data
    ]
    ...
    for i, layer in enumerate(hparams.layers):
        ...
        for k, idxs in idxs_dict.items():
            ...
            for idx in idxs:
                ...
                ks_list.append(layer_in_ks[k, idx + (contexts_tok['attention_mask'][k] == 0).sum()])
            for unselected_idx in unselected_idxs:
                if unselected_idx - (contexts_tok['attention_mask'][k] == 0).sum() >= 0:
                    kp_list.append(layer_in_ks[k, unselected_idx - (contexts_tok['attention_mask'][k] == 0).sum()])
